...
public class CenterHeuristic implements Heuristic<Solution> {
	...
	public Solution apply(Solution solution) {
		...
		for(int i = 0; i < solution.getCircleCount(); i++) {
			maxX = Math.max(maxX, solution.getCircle(i).getPosition().x + solution.getCircle(i).getRadius());
			maxY = Math.max(maxY, solution.getCircle(i).getPosition().y + solution.getCircle(i).getRadius());
			minX = Math.min(minX, solution.getCircle(i).getPosition().x - solution.getCircle(i).getRadius());
			minY = Math.min(minY, solution.getCircle(i).getPosition().y - solution.getCircle(i).getRadius());
		}
		Point2D.Double shift = Operation.invert(new Point2D.Double((maxX + minX) / 2, (maxY + minY) / 2));
